...
- **Four absolute URLs in `<head>` hard-code the origin**: `canonical`,
  `og:url`, `og:image` and `twitter:image`, all pointing at
  `devwithfarshi.github.io/driftgram-sync-tool`. Change all four together when
  the site gets its real domain. They cannot be relative — every unfurler
  drops a relative `og:image` and shows no card at all, which is the failure
  this replaced.
- The card image is `assets/social-preview.png`, 1280×640 (the 2:1 that
  `summary_large_image` wants), regenerated by
  `python tools/make_brand_assets.py`. `og:image:width`/`height` in the markup
